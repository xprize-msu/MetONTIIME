...
def read_taxa_file_list(file_path):
    """
    Reads a text file where each line is foramt as 'file_path,id_threshold,db' and stores
    ...
    for ln in sample_lines:
        split_ln = ln.split(",")
        file_and_ids[split_ln[0]] = [split_ln[1].strip(),split_ln[2].strip()]
...
def do_update_taxa(row):
    ...
    # Flag taxa with genus = family = order
    if "g" in taxa_dict and taxa_dict["g"] == taxa_dict["f"] and taxa_dict["g"] == taxa_dict["o"]:
        update_status="Genus=family=order; possible unresolved taxa"
    ...
    if "s" in taxa_dict:
        ...
        # Fix species 
        elif not "_" in taxa_dict["s"]:
             update_s = taxa_dict["g"] + "_" + taxa_dict["s"]
             taxa_dict["s"] = update_s
             update_status="Convert species to binomial format with genus"
...
def main():
    ...
    for f in tax_file_list:
        ...
        tax_df["file"] = f
        tax_df["id"] = float(files_and_ids[f][0])
        tax_df["db"] = files_and_ids[f][1]
        prio = "NA"
        if files_and_ids[f][1] == "Custom_18S":
            prio = 1
        elif files_and_ids[f][1] == "SILVA":
            prio = 0
        tax_df["db_prio"] = prio 
    ...
    print(merge_df["id"].value_counts())
    print(merge_df["db"].value_counts())
    filter_df = merge_df.drop(merge_df[merge_df['Taxon'] == 'Unassigned'].index)
    print("Remove Unassigned")
    print(filter_df.shape)
    print(filter_df["id"].value_counts())
    print(filter_df["db"].value_counts())
    filter_df = filter_df.sort_values(['db_prio',"id"]).drop_duplicates(subset='Feature ID', keep='last')
    print("Filter by id percent and DB")
    print(filter_df.shape)
    print(filter_df["id"].value_counts())
    print(filter_df["db"].value_counts())
